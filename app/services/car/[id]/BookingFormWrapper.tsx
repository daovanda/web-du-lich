...
export default function BookingFormWrapper({
  ...
  // Calculate quick estimate (giống tour: giá/người * số người)
  ...
  return (
    <section className="bg-gradient-to-br from-blue-900/20 to-purple-900/20 rounded-2xl p-6 border border-blue-700/50 sticky top-24">
      <h2 className="text-2xl font-bold mb-6">Đặt xe ngay</h2>

      {!showForm ? (
        <>
          {/* Price display */}
          <div className="mb-6">
            <p className="text-sm text-gray-400 mb-2">Giá xe</p>
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className={`w-full rounded-lg px-4 py-3 focus:outline-none transition-colors ${
                quickCalcPeople &&
                (isNaN(parseInt(quickCalcPeople)) ||
                  parseInt(quickCalcPeople) < 1 ||
                  parseInt(quickCalcPeople) > 999)
            ...
            {quickCalcPeople &&
              !isNaN(parseInt(quickCalcPeople)) &&
              parseInt(quickCalcPeople) < 1 && (
                <p className="mt-2 text-sm text-red-400">⚠️ Số người phải lớn hơn 0</p>
              )}
            {quickCalcPeople &&
              !isNaN(parseInt(quickCalcPeople)) &&
              parseInt(quickCalcPeople) > 999 && (
                <p className="mt-2 text-sm text-red-400">⚠️ Số người tối đa là 999</p>
              )}
