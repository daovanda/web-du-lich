...
export interface StayFiltersProps {
  ...
  initialFilters?: Partial<StayFilterState>;
...
export interface StayFilterState {
  ...
  minBeds: string;
  maxBeds: string;
  priceRange: string; // "all" | "under1m" | "1m-3m" | "over3m"
  checkInDate: string;
  checkOutDate: string;
}
...
export default function StayFilters({ onFiltersChange, initialFilters, isInitialLoad = false }: StayFiltersProps) {
  const [filters, setFilters] = useState<StayFilterState>({
    searchQuery: initialFilters?.searchQuery || "",
    location: initialFilters?.location || "",
    minGuests: initialFilters?.minGuests || "0",
    ...
    maxBeds: initialFilters?.maxBeds || "",
  ...
  const minCheckOutDate = filters.checkInDate 
    ? new Date(new Date(filters.checkInDate).getTime() + 86400000).toISOString().split('T')[0]
